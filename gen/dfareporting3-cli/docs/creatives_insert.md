...
You can set the scope for this method like this: `dfareporting3 --scope <scope> creatives insert ...`
...
* `rendering-id-dimension-value    dimension-name=sit`
    - The name of the dimension.
* `etag=at`
    - The eTag of this response for caching purposes.
* `id=voluptua.`
...
* `..    required-flash-plugin-version=kasd`
...
* `..skip-offset    offset-percentage=79`
    - Duration, as a percentage of video duration. Do not set when offsetSeconds is set. Acceptable values are 0 to 100, inclusive.
* `offset-seconds=83`
    - Duration, in seconds. Do not set when offsetPercentage is set. Acceptable values are 0 to 86399, inclusive.
...
* `studio-advertiser-id=sadipscing`
...
* `studio-trafficked-creative-id=est`
...
* `universal-ad-id    registry=clita`
