...
impl<'a, C, A> ArchiveInsertCall<'a, C, A> where C: BorrowMut<hyper::Client<hyper::client::HttpConnector, hyper::Body>>, A: oauth2::GetToken {
    ...
    fn doit<RS>(mut self, mut reader: RS, reader_mime_type: mime::Mime, protocol: &'static str) -> Box<Future<Item = Result<(hyper::Response<hyper::Body>, Groups)>, Error = cmn::Error>>
		where RS: ReadSeek {
            ...
            let mut req_fut: hyper::client::ResponseFuture = {
                if should_ask_dlg_for_url && (upload_url = dlg.upload_url()) == () && upload_url.is_some() {
                    ...
                    *req.uri_mut() = url.clone();
                    ...
                    if size > 26214400 {
                    	return Box::new(futures::future::err(Error::UploadSizeLimitExceeded(size, 26214400)))
                    }
                        {
                            let headers_mut = req.headers_mut();
...
*/
    ...
    pub fn upload<RS>(self, stream: RS, mime_type: mime::Mime) -> Box<Future<Item = Result<(hyper::Response<hyper::Body>, Groups)>, Error = cmn::Error>>
                where RS: ReadSeek {
    ...
    /// * *multipart*: yes
    /// * *valid mime types*: 'message/rfc822'
    pub fn upload_resumable<RS>(self, resumeable_stream: RS, mime_type: mime::Mime) -> Box<Future<Item = Result<(hyper::Response<hyper::Body>, Groups)>, Error = cmn::Error>>
                where RS: ReadSeek {
