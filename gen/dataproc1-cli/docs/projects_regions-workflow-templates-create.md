...
You can set the scope for this method like this: `dataproc1 --scope <scope> projects regions-workflow-templates-create ...`
...
* `..master-config.disk-config    boot-disk-size-gb=15`
...
* `managed-group-config    instance-group-manager-name=amet`
...
* `..secondary-worker-config.disk-config    boot-disk-size-gb=57`
    - Optional. Size in GB of the boot disk (default is 500GB).
* `boot-disk-type=clita`
    - Optional. Type of the boot disk (default is &#34;pd-standard&#34;). Valid values: &#34;pd-ssd&#34; (Persistent Disk Solid State Drive) or &#34;pd-standard&#34; (Persistent Disk Hard Disk Drive).
* `num-local-ssds=78`
    - Optional. Number of attached SSDs, from 0 to 4 (default is 0). If SSDs are not attached, the boot disk is used to store runtime logs and HDFS (https://hadoop.apache.org/docs/r1.2.1/hdfs_user_guide.html) data. If one or more SSDs are attached, this runtime bulk data is spread across them, and the boot disk contains only basic config and installed binaries.
...
* `is-preemptible=false`
    - Optional. Specifies that this instance group contains preemptible instances.
* `machine-type-uri=sed`
    - Optional. The Compute Engine machine type used for cluster instances.A full URL, partial URI, or short name are valid. Examples:
        https://www.googleapis.com/compute/v1/projects/[project_id]/zones/us-east1-a/machineTypes/n1-standard-2
        projects/[project_id]/zones/us-east1-a/machineTypes/n1-standard-2
        n1-standard-2Auto Zone Exception: If you are using the Cloud Dataproc Auto Zone Placement feature, you must use the short name of the machine type resource, for example, n1-standard-2.
* `managed-group-config    instance-group-manager-name=sanctus`
    - Output only. The name of the Instance Group Manager for this group.
...
* `..software-config    image-version=gubergren`
...
* `..worker-config.disk-config    boot-disk-size-gb=72`
    - Optional. Size in GB of the boot disk (default is 500GB).
* `boot-disk-type=sit`
    - Optional. Type of the boot disk (default is &#34;pd-standard&#34;). Valid values: &#34;pd-ssd&#34; (Persistent Disk Solid State Drive) or &#34;pd-standard&#34; (Persistent Disk Hard Disk Drive).
* `num-local-ssds=62`
...
* `machine-type-uri=gubergren`
    - Optional. The Compute Engine machine type used for cluster instances.A full URL, partial URI, or short name are valid. Examples:
        ...
        projects/[project_id]/zones/us-east1-a/machineTypes/n1-standard-2
        n1-standard-2Auto Zone Exception: If you are using the Cloud Dataproc Auto Zone Placement feature, you must use the short name of the machine type resource, for example, n1-standard-2.
* `managed-group-config    instance-group-manager-name=est`
    - Output only. The name of the Instance Group Manager for this group.
* `instance-template-name=invidunt`
    - Output only. The name of the Instance Template used for the Managed Instance Group.
