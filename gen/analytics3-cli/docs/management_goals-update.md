...
* `event-details    use-event-value=true`
    - Determines if the event value should be used as the value for this goal.

* `..    id=consetetur`
    - Goal ID.
* `internal-web-property-id=tempor`
    - Internal ID for the web property to which this goal belongs.
* `kind=amet`
    - Resource type for an Analytics goal.
* `name=lorem`
    - Goal name.
* `parent-link    href=eirmod`
...
* `url-destination-details    case-sensitive=false`
    - Determines if the goal URL must exactly match the capitalization of visited URLs.
* `first-step-required=true`
...
* `..    value=0.965116885387`
    - Goal value.
* `visit-num-pages-details    comparison-type=gubergren`
    - Type of comparison. Possible values are LESS_THAN, GREATER_THAN, or EQUAL.
* `comparison-value=eos`
    - Value used for this comparison.

* `..visit-time-on-site-details    comparison-type=sed`
    - Type of comparison. Possible values are LESS_THAN or GREATER_THAN.
* `comparison-value=sit`
    - Value used for this comparison.

* `..    web-property-id=eirmod`
