...
You can set the scope for this method like this: `dlp2 --scope <scope> projects inspect-templates-create ...`
...
* `-r .inspect-template    create-time=lorem`
    - The creation timestamp of a inspectTemplate, output only field.
* `description=clita`
    - Short description (max 256 chars).
* `display-name=invidunt`
    - Display name (max 256 chars).
* `inspect-config    content-options=eirmod`
...
* `limits    max-findings-per-item=79`
    - Max number of findings that will be returned for each item scanned.
        When set within `InspectDataSourceRequest`,
        the maximum returned is 1000 regardless if this is set higher.
        When set within `InspectContentRequest`, this field is ignored.
* `max-findings-per-request=21`
